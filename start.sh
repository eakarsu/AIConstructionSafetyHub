...
echo -e "${CYAN}╔══════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║   AI Construction Safety Hub - OSHA Compliance       ║${NC}"
echo -e "${CYAN}╚══════════════════════════════════════════════════════╝${NC}"
echo ""
...
pkill -9 -f "AIConstructionSafetyHub/backend.*nodemon"        2>/dev/null || true
pkill -9 -f "AIConstructionSafetyHub/backend.*server\.js"     2>/dev/null || true
pkill -9 -f "AIConstructionSafetyHub/frontend.*react-scripts" 2>/dev/null || true
...
psql -h ${DB_HOST:-localhost} -p ${DB_PORT:-5432} -U ${DB_USER:-postgres} -tc "SELECT 1 FROM pg_database WHERE datname = '${DB_NAME:-construction_safety_hub}'" 2>/dev/null | grep -q 1 || \
  psql -h ${DB_HOST:-localhost} -p ${DB_PORT:-5432} -U ${DB_USER:-postgres} -c "CREATE DATABASE ${DB_NAME:-construction_safety_hub}" 2>/dev/null || \
  createdb -h ${DB_HOST:-localhost} -p ${DB_PORT:-5432} -U ${DB_USER:-postgres} ${DB_NAME:-construction_safety_hub} 2>/dev/null || true
